    ...
    if( (c&0xff)==0xef && p->bNotFirst==0 ){
      ...
      if( (c&0xff)==0xbb ){
        ...
        if( (c&0xff)==0xbf ){
          p->bNotFirst = 1;
          p->n = 0;
          return tbl_read_one_field(p, delim);
        }
      }
    }
    while( c>delim || (c!=EOF && c!=delim && c!='\n') ){
      if( csv_append(p, (char)c) ) return 0;
      c = csv_getc(p);
    }
    if( c=='\n' ){
  ...
  *ppVtab = (sqlite3_vtab*)pNew;
  ...
  if( CSV_SCHEMA==0 ){
    ...
    if( nCol>0 && bHeader<1 ){
      ...
      do{
        char *z = tbl_read_one_field(&sRdr,',');
    ...
    if( CSV_SCHEMA==0 ) goto tbltab_connect_oom;
  }else if( nCol<0 ){
    do{
      tbl_read_one_field(&sRdr,TBL_FDELIM);
      pNew->nCol++;
    }while( sRdr.cTerm==TBL_FDELIM );
  }else{
    pNew->nCol = nCol;
  }
